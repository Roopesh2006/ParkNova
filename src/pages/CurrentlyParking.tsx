...
export default function CurrentlyParking() {
  const navigate = useNavigate();
  const [sorting, setSorting] = React.useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = React.useState('');
  const [parkingEntries, setParkingEntries] = React.useState<ParkingEntry[]>([]);
  const [isLoading, setIsLoading] = React.useState(true);

  React.useEffect(() => {
    const fetchEntries = async () => {
      try {
        const data = await apiFetch('/api/parking/sessions/active');
  ...
  const columnHelper = createColumnHelper<ParkingEntry>();
  ...
  const columns = [
    ...
    columnHelper.accessor('in_time', {
      ...
      cell: info => (
        <div className="space-y-1">
          <div className="text-sm text-text-1">{new Date(info.getValue()).toLocaleTimeString()}</div>
    ...
    columnHelper.display({
      ...
      cell: info => (
        <div className="flex items-center gap-2">
          <button 
            onClick={() => navigate(`/parking/parking/${info.row.original.id}/end`)}
            className="flex items-center gap-2 px-4 py-2 bg-red/10 text-red border border-red/20 rounded-lg text-xs font-bold hover:bg-red hover:text-white transition-all"
          >
            <LogOut size={14} />
            Checkout
          </button>
  ...
  return (
    <div className="space-y-8 page-enter">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-display font-black text-text-1 tracking-tight">Currently Parking</h2>
          <p className="text-text-3">Monitor and manage vehicles currently in the parking lot</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-primary/10 border border-primary/20 rounded-xl flex items-center gap-2 text-primary font-bold">
            <Car size={18} />
            <span>{parkingEntries.length} Vehicles</span>
          </div>
          <button 
            onClick={() => navigate('/parking/parking-crud/create')}
      ...
      <div className="bg-bg-card border border-border rounded-2xl overflow-hidden shadow-xl">
        <div className="p-6 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-4 bg-bg-surface/50">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search vehicle, place or slot..."
              value={globalFilter}
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-12 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-text-3 font-medium">Loading active sessions...</p>
            </div>
            ...
            <div className="p-12 flex flex-col items-center justify-center space-y-4 text-center">
              ...
              <div>
                <h3 className="text-lg font-bold text-text-1">No active sessions</h3>
                <p className="text-text-3 max-w-xs mx-auto">There are currently no vehicles parked in the system.</p>
              </div>
              <button 
                onClick={() => navigate('/parking/parking-crud/create')}
        ...
        {!isLoading && parkingEntries.length > 0 && (
