...
import { VehicleCategory } from '../types/index';
import { toast } from 'sonner';
import { apiFetch } from '../lib/api-client';

const columnHelper = createColumnHelper<VehicleCategory>();
...
export default function CategoryList() {
  ...
  const columns = [
    ...
    columnHelper.accessor('sort_order', {
      header: 'Sort Order',
      cell: info => <span className="text-text-2">{info.getValue()}</span>,
    }),
    columnHelper.display({
      ...
      cell: (info) => (
        <div className="flex items-center gap-2">
          <button 
            onClick={() => navigate(`/parking/category/edit/${info.row.original.id}`)}
  ...
  return (
    <div className="space-y-6 page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-display font-black text-text-1 tracking-tight">Category Management</h2>
          <p className="text-text-3">Define vehicle types and categories for your facility</p>
        </div>
        <button 
          onClick={() => navigate('/parking/category/create')}
      ...
      <div className="bg-bg-card border border-border rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-20 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-text-3 font-medium">Loading categories...</p>
            </div>
          ) : categories.length === 0 ? (
            <div className="p-20 text-center">
              ...
              <h3 className="text-xl font-bold text-text-1 mb-2">No categories found</h3>
              <p className="text-text-3 mb-6">Start by adding your first vehicle category.</p>
              <button 
                onClick={() => navigate('/parking/category/create')}
        ...
        <div className="p-6 flex items-center justify-between">
          <div className="text-sm text-text-3">
            Showing <span className="text-text-1 font-bold">{table.getRowModel().rows.length}</span> of <span className="text-text-1 font-bold">{categories.length}</span> categories
          </div>
          <div className="flex items-center gap-2">
            <button 
